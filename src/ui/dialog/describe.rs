...
impl DescribeDialog {
    pub fn new(revset: &str, text: String) -> Self {
    ...
    fn match_event(&self, event: &Event) -> Action {
        if let Event::Key(key) = event {
            return self.keybinds.match_event(*key).unwrap_or(Action::None);
        }
        Action::None
    }
}
    ...
    fn input(&mut self, event: Event) -> Result<ComponentInputResult> {
        match self.match_event(&event) {
            Action::Save => {
                // TODO: Handle error
                new_commander()
                    .run_describe(&self.revset, &self.describe_textarea.lines().join("\n"))?;
